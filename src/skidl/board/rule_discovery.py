...
The rule ladder (first hit wins, per setting):
  ...
  2. project .kicad_pro (Board Setup / net-class dialogs)    "project-user"
     + project .kicad_dru custom rules file (detected, preserved)
  3. existing .kicad_pcb setup values                        "board-user"
...
def read_board_setup(pcb_path) -> dict:
    """User-relevant blocks from an existing .kicad_pcb: the raw (setup ...)
...
def discover_rules(base: str, out_dir) -> dict:
    ...
    return {
        "project": {
            ...
            "net_classes": project_classes,          # user/AI-set, in .kicad_pro
