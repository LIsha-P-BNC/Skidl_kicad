...
"""
Post-process a KiCad 8/9 schematic (.anvil_sch) generated by SKiDL to add
...
Usage as CLI::
    ...
    python -m skidl.tools.inject_labels schematic.anvil_sch netlist.net output.anvil_sch

Usage from Python::

    from skidl.tools.inject_labels import inject_labels
    inject_labels("power1.anvil_sch", "circuit.net", "power1_labeled.anvil_sch")
"""
...
def inject_labels(sch_path: str, net_path: str, out_path: str) -> int:
    ...
    Args:
        sch_path: Path to a ``.anvil_sch`` file (input).
        net_path: Path to a ``.net`` netlist file.
        out_path: Path for the labeled ``.anvil_sch`` file (output).
...
def main() -> None:
    if len(sys.argv) != 4:
        print(
            ...
            "<schematic.anvil_sch> <netlist.net> <output.anvil_sch>",
            file=sys.stderr,
        )
        sys.exit(1)
