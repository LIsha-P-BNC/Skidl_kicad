...
def _power_symbol_to_sexp(pin, net_name, tx):
    ...
    if not (math.isfinite(pt.x) and math.isfinite(pt.y)):
        ...
        _warnings.warn(
            "skidl: skipping power symbol '{}' at non-finite position -- "
            "unplaced pin".format(net_name),
            RuntimeWarning,
        )
        return None
    ...
    # Power-symbol orientation: the UNIVERSAL EDA convention is a FIXED heading --
    # a GROUND symbol (GND/AGND/DGND/PGND/VSS/VEE) always points DOWN and a VOLTAGE
    # rail (+xV/VCC/VDD/VBUS/VBAT...) always points UP, regardless of which side of
    # the part the pin is on. A short wire routes from the pin to the conventionally
    # oriented symbol. KiCad's own power symbols already draw this at angle 0
    # (voltage up, ground down), so angle is ALWAYS 0.
    #
    # (An earlier pin-aware rotation -- turning the symbol to face the pin's exit
    # direction -- was WRONG: it left GND symbols pointing sideways on side/top
    # pins, which reads as broken across many circuits. Orientation is fixed by
    # symbol TYPE, not pin direction; only the stub PLACEMENT follows the pin.)
    angle = 0
...
def net_label_to_sexp(pin, tx=Tx(), force=False):
    ...
    symbol. Other labels are local on one sheet and global across sheets.
    ...
    # A local label is sufficient on one sheet. Reserve global labels for
    # nets whose real pins span hierarchy modules; global labels otherwise
    ...
    label_fields = [label_type, pin.net.name]
    if label_type == "global_label":
        label_fields.append(["shape", "bidirectional"])
    label_fields.extend(
        [
...
# Optional title-block metadata (revision, company, engineer, date, project),
# set once per build by smart_schematic via set_title_block_meta(). Kept as module
# state so the fields reach create_title_block_sexp without threading extra kwargs
# through generate_schematic -> write_top_schematic -> node_to_sexp_schematic.
_TITLE_META = {}


@export_to_all
def set_title_block_meta(**meta):
    """Set optional title-block fields for the next schematic generation.

    Recognized keys: rev, company, engineer, date, project. Empty values are
    ignored (the field falls back to its default). Call before generate_schematic.
    """
    _TITLE_META.clear()
    _TITLE_META.update({k: str(v) for k, v in meta.items() if v})


def create_title_block_sexp(title):
    """Create a title block S-expression (title, date, rev, company, engineer).
    ...
    # ["rev", ""] renders as bare "(rev)" -- which KiCad rejects with
